...
| R003 | M1 | FQ lifecycle unit tests | KIVI4-FQ/KIVI2-FQ | synthetic + short prompt | zero FP residual prompt tokens, shape, finite, packed metadata | MUST | PASS | Unit + CUDA smoke pass; chưa phải 20-dev gate |
| R004 | M1 | Native/dequantized cache correctness | KIVI4/2 STD and FQ | 20 dev | K/V MSE, cosine, relative L2, storage bytes | MUST | PASS | 1 real Llama cache materialized; full 20-dev report còn TODO |
...
| R006 | M3 | Minimum FQ leakage screen, Protocol A | FP16 vs KIVI4-FQ/KIVI2-FQ | 20 dev | semantic, ROUGE-L, token acc, exact match, runtime | MUST | PASS | 1-sample inversion + Collision smoke pass; chưa phải Gate G3 |
...
| 2026-09-09 | implementation smoke | `PASS` for runtime wiring only | FQ 4-bit CUDA roundtrip, one real Llama cache, one-sample inversion and Collision logs; `fp_residual_prompt_tokens=0` | Create a sufficiently long stratified dataset, then run G0–G3 on 20 dev samples |
